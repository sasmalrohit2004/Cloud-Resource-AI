...
export const api = {
  ...
  createAlert: async (alert) => {
    try {
      const res = await apiClient.post('/alerts', alert);
      return res.data;
    } catch {
      return alert;
    }
  ...
  resolveAlert: async ({ timestamp, type, status = "RESOLVED", resolved_by = "Site Reliability Engineer" }) => {
    try {
      const res = await apiClient.post('/alerts/resolve', {
        timestamp,
        type,
        status,
        resolved_by
      ...
      return { status: "success", resolved_count: 1 };
    }
  },

  resolveAllAlerts: async (resolved_by = "Site Reliability Engineer") => {
    try {
      const res = await apiClient.post('/alerts/resolve-all', { resolved_by });
      return res.data;
    } catch {
      return { status: "success", resolved_count: 1 };
    }
